...
** Sets sensible defaults for a location block:
** - No redirects (code 0)
...
** Sets sensible defaults for a server block:
** - Listen on all interfaces (0.0.0.0) port 8080
** - 1MB max body size (prevents memory exhaustion from large uploads)
** - No custom error pages (will use defaults)
*/
ServerConfig::ServerConfig()
    : host("0.0.0.0"),
      port(8080),
      client_max_body_size(1048576)  // 1MB default, same as many web servers
{
}
...
**
** This is how NGINX works, and we replicate that behavior.
*/
...
** We use the current directory for simplicity.
*/
...
/*
** Constructor with explicit config path
** This is used when the user specifies a config file on the command line:
...
** We need to remove everything after # to parse correctly
*/
...
void Config::parseServerBlock(std::ifstream& file, std::string& line) {
    ...
    while (std::getline(file, line)) {
        ...
        if (directive == "listen") {
            ...
            ** Used for name-based virtual hosting (out of scope per subject,
            ** but useful for routing if implemented)
...
void Config::parseLocationBlock(std::ifstream& file, std::string& line, ServerConfig& server) {
    ...
    while (std::getline(file, line)) {
        ...
        if (directive == "root") {
            ...
            ** This is crucial for security - only allow necessary methods
            */
            ...
            */
            if (tokens.size() < 2) {
                throw ConfigException("autoindex directive requires on/off");
            }
